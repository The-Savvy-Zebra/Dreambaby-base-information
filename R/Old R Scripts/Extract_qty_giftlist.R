...
raw_data <-
  ...
  as_tibble() %>%
  pivot_longer(cols = 4:last_column,names_to = "date", values_to = "qty_giftlist") %>%
  
  # Convert the dates
  mutate(date = excel_numeric_to_date(as.numeric(as.character(date)), date_system = "modern")) %>%
  
  filter(!is.na(qty_giftlist)) %>%
  ...
  summarise(qty_giftlist=sum(qty_giftlist,na.rm=TRUE)) %>%
  ungroup() %>%
  
  # put the columns in a logical order
  select(date,kostenplaats, region,  winkel, shop , qty_giftlist)

raw_data

# Save data to feather
write_feather(raw_data,"qty_giftlist_products_by_shop.feather")
