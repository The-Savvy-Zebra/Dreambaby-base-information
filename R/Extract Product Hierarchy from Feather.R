...
for(i in 1:NROW(webshop_dc_file_names)) {
  ...
  if(i==1) {
    product_structure <- df
  } else {
    product_structure <- 
      rbind(product_structure,df) %>%
      unique()
  }
  
  stopTextInGreen(paste0("Processing file ",webshop_dc_file_names$x[i]))
  
  print(NROW(product_structure))
}
...
for(i in 1:NROW(shop_file_names)) {
  ...
  print(paste0("Distinct products found till now ",NROW(product_structure)))
}


# Save data to feather
write_feather(product_structure,"../Feather Files/product_hierarchy.feather")
